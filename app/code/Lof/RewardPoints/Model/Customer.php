...
class Customer extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $totalPoints = (float) $this->getTotalPoints();
        // $this->rewardsLogger->info('Total points available: ' . $totalPoints);
        
        // Only get purchases with order_status = 1 (active)
        ...
        foreach ($purchaseCollectionFactory as $purchase) {
            $purchasePoints = (float)$purchase->getSpendPoints();
            $spendPoints += $purchasePoints;
            // $this->rewardsLogger->info('Adding spend points from purchase ID: ' . $purchase->getId() . ', amount: ' . $purchasePoints);
        }
        
        // $this->rewardsLogger->info('Total spend points calculated: ' . $spendPoints);

        // Calculate available points
        if ($spendPoints > $totalPoints) {
            // $this->rewardsLogger->addError('Warning: Total Points is less than spend points - totalPoints: ' . $totalPoints . ', spendPoints: ' . $spendPoints);
            $availablePoints = 0;
        } else {
            $availablePoints = (float) ($totalPoints - $spendPoints);
        }
        
        // $this->rewardsLogger->info('Final available points set: ' . $availablePoints);
