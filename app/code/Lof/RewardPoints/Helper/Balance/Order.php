...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * @var \Lof\Subtotal\Helper\Data
     */
    protected $subTotalHelper;
     ...
     * @param \Lof\Subtotal\Helper\Data                  $subTotalHelper
     */
    public function __construct(
        ...
        \Lof\Subtotal\Helper\Data $subTotalHelper,
        ...
        $this->subTotalHelper  = $subTotalHelper;
    }

    public function calculateEarnPoints($order, $purchase = null)
    {
        try {
            $totalEarnedPoints = round((float) $order->getSubtotal());
            $rewardPointsDiscount = round(abs((float) $purchase->getDiscount()));
            $couponDiscount = round(abs((float) $order->getDiscountAmount()));

            $finalEarnPoints = max(0, $totalEarnedPoints - $rewardPointsDiscount - $couponDiscount);

            return $finalEarnPoints;
        } catch (\Exception $e) {
            $this->rewardsLogger->error('Error in calculateEarnPoints: ' . $e->getMessage(), [
                'order_id' => $order->getId(),
                'increment_id' => $order->getIncrementId(),
                'subtotal' => $order->getSubtotal(),
                'reward_points_discount' => $order->getRewardpointsDiscount(),
    ...
    public function proccessOrder($order, $adminId = '', $purchase = null)
    {
        try {
            ...
            if (!$this->subTotalHelper) {
                $totalEarnedPoints = (float) $purchase->getEarnPoints();
            } else {
                $totalEarnedPoints = $this->calculateEarnPoints($order, $purchase);
            }
            ...
            if ($totalEarnedPoints > 0) {
                ...
                $params = [
                    ...
                    'amount_used'   => 0,
                    'title'         => __('Earned %1 for the order #%2', $this->rewardsData->formatPoints($totalEarnedPoints), $order->getIncrementId()),
                    'code'          => $code,
    ...
    public function earnOrderPoints($order, $withAdmin = true)
    {
        try {
            ...
            if ($transaction->getId() && $transaction->getStatus() != Transaction::STATE_COMPLETE) {
                 ...
                 */
                $transaction->setAction(Transaction::EARNING_ORDER)
                ->setStatus(Transaction::STATE_COMPLETE)
                ->setIsapplied(1) // Set isapplied to 1 for earn points
                ->save();
                $purchase = $this->rewardsPurchase->getByOrder($order);
                $purchase->setData('order_status', 0)->save();
    ...
    public function spendOrderPoints($order, $withAdmin = true)
    {
        try {
            ...
            if ($transaction->getId() && $transaction->getStatus() == Transaction::STATE_PROCESSING) {
                 ...
                 * Change transaction status to Complete
                 */
                $transaction->setAction(Transaction::SPENDING_ORDER)
                ->setStatus(Transaction::STATE_COMPLETE)
                ->setIsapplied(1)
                ->save();
                $purchase = $this->rewardsPurchase->getByOrder($order);
