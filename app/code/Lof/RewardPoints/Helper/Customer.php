...
class Customer extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param $customerId
     * @param $params
     * @return \Lof\RewardPoints\Model\Customer|\Magento\Framework\DataObject|mixed|void
     * @throws \Exception
     */
...
//            $availablePoints = $customer->getAvailablePoints();
            ...
            foreach ($transaction as $item) {
                $availablePoints += $item->getAmount();
            ...
            $spendProductPoints = $purchaseParams[Config::SPENDING_PRODUCT_POINTS]['items'] ?? [];
            foreach ($items as $item) {
            ...
            $maxAvaiableItem = (float)($availablePoints / $newItem['points']);
            ...
            return $result;
        } elseif ((!is_array($spendingProductPoints) && $spendingProductPoints > 0) || (is_array($spendingProductPoints) && count($spendingProductPoints) > 0)) {
    ...
    public function proccessRule($params, $result, $product)
    {
        ...
        } elseif ((isset($params['discount']) && (float) $params['discount'] > 0) && (isset($params['spendpoints']) && (float)$params['spendpoints'] > 0) && isset($params['rule'])) {
    ...
    public function transactionExpired($customerId)
    {
        ...
        $collection  = $this->transactionCollectionFactory->create()
            ...
            ->addFieldToFilter('expires_at', [['null' => true],['gt' => $today]]);
        return $collection;
    }
}
