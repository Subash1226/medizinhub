...
class Update extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if (!empty($sellerId)) {
            ...
            $this->messageManager->addSuccess(__('You don\'t have permission to proceed with this action'));
            $resultRedirect = $this->resultRedirectFactory->create();
    ...
    public function createInvoice($order, $itemsArray, $productSkus, $shippingAmount, $subTotal, $baseSubtotal, $shipVirtualFlag) {
        if ($order->canInvoice()) {
            ...
            $order->addStatusHistoryComment(' | ' . __('Invoice') . ' #' . $invoice->getIncrementId() . ' | ' . $successMsg)->setIsCustomerNotified(true)->save();

            // Change seller order status
            $sellerOrderCollection = $this->_objectManager->get('Apptha\Marketplace\Model\Order')->getCollection()
                ->addFieldToFilter('order_id', $order->getId())
                ->addFieldToFilter('seller_id', $customerSession->getId());

            if ($sellerOrderCollection->getSize() > 0) {
                $sellerOrder = $sellerOrderCollection->getFirstItem();
                $totalAmount = $sellerOrder->getSellerAmount() + $sellerOrder->getShippingAmount();
