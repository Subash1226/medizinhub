...
use Apptha\Marketplace\Helper\Marketplace as MarketplaceHelper;
...
class Savedata extends Action
{
    ...
    public function execute() {
        ...
        try {
            ...
            if ($e->getMessage() == 'A product with the same SKU already exists.') {
        ...
        $product->setData('product_details', $productData ['product_details']);
        $product->setData('quantity_contents', $productData ['quantity_contents']);
        $product->setData('prescription_check', $productData ['prescription_check']);
        $product->setData('category_check', $productData ['category_check']);
        $product->setData('manufacturer', $productData ['manufacturer']);
        $product->setData('hsn_code', $productData ['hsn_code']);
        $product->setData('expiry', $productData ['expiry']);
        $product->setData('uses_of_product', $productData ['uses_of_product']);
        $product->setData('direction_for_use', $productData ['direction_for_use']);
        $product->setData('side_effects', $productData ['side_effects']);
        $product->setData('safety_advices', $productData ['safety_advices']);
        $product->setData('additional_details', $productData ['additional_details']);
        ...
        if (! empty ( $productId )) {
            ...
            if ($urlKeyCount >= 1) {
                $urlKey = $productData['name'] . '-' . rand(1, 10000);
                $product->setUrlKey($urlKey);
            } else {
                $product->setUrlKey($productData['name']);
            }
            $product->save ();
            if(!empty($this->systemHelper->getYoutubeKey())) {
                $product = $this->dataHelper->addVideo($product,$postValue);
            ...
            foreach ($configurableProducts as $configurableProduct) {
                if (isset($configurableProduct['attribute_combination'])) {
                    ...
                    if (isset($configurableProduct['simple_product_id'])) {
                        ...
                        $simpleProduct->setWebsiteIds(array($websiteId));
